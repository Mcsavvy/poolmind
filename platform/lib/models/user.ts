import { createModel, IBaseDocument, commonFields, validators } from '@/lib/schemas';
...
const User = createModel<IUser>('User', {
  ...
  additionalMethods: {
    ...
    getDisplayName(this: IUser): string {
      return this.displayName || this.username || `${this.walletAddress.substring(0, 8)}...`;
    },
    ...
    addConnection(this: IUser, walletType: string, ipAddress?: string): Promise<IUser> {
    ...
    updateNotificationPreferences(this: IUser, preferences: Partial<INotificationPreferences>): Promise<IUser> {
      ...
      const stats = await this.aggregate([
        { $match: { isActive: true } },
        {
          $group: {
            ...
            usersWithUsername: { $sum: { $cond: [{ $ne: ['$username', null] }, 1, 0] } },
  ...
  indexes: [
    [{ walletAddress: 1, isActive: 1 }, { unique: true }],
    [{ username: 1, isActive: 1 }, { unique: true }],
    [{ email: 1, isActive: 1 }, { unique: true }],
    [{ role: 1, isActive: 1 }, { unique: true }],
    [{ lastLoginAt: -1 }, { unique: true }]
  ]
});
