...
export default function WalletPage() {
  ...
  const stxBalanceUSD = userBalance
    ? (Number(userBalance.stxBalance) / 1_000_000) * stxPrice
    : 0;
  const poolShareUSD = userBalance
    ? (Number(userBalance.poolShareValue) / 1_000_000) * stxPrice
    : 0;

  // Calculate P&L percentage if we have stats
  const pnlPercentage =
    userStats && Number(userStats.totalDeposited) > 0
      ...
      <div className='flex flex-col space-y-8'>
              ...
              {isLoadingBalance ? (
                ...
                <>
                  ...
                  {!isLoadingStats && userStats && (
                    <p
                      className={`text-sm mt-2 flex items-center gap-1 ${pnlFormatted.colorClass}`}
                    >
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Current pool metrics and your position
            </CardDescription>
          </CardHeader>
          <CardContent>
            {isLoadingPool ? (
              ...
              <div className='flex items-center justify-center p-8'>
                <p className='text-muted-foreground'>
                  Failed to load pool information
                </p>
              </div>
            )}
          </CardContent>
        ...
        {!isLoadingStats &&
          userStats &&
          Number(userStats.totalDeposited) > 0 && (
            <Card>
              <CardHeader>
                <CardTitle>Your Performance</CardTitle>
                <CardDescription>
                  Overview of your pool investment performance
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className='grid grid-cols-2 md:grid-cols-4 gap-4'>
                  ...
                  <div>
                    ...
                    <p
                      className={`text-lg font-semibold ${pnlFormatted.colorClass}`}
                    >
