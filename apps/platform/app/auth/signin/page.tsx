        ...
        {session ? (
          <Card>
            <CardHeader className="space-y-1">
              <CardTitle className="text-2xl text-center">Already signed in</CardTitle>
            ...
            <CardContent className="space-y-4 flex flex-col items-center">
              <div className="flex flex-col items-center space-y-2">
                <WalletAvatar
                  ...
                  size="lg"
                  username={session?.user.username || session?.user.displayName || session?.user.email}
                  profilePicture={session.user.profilePicture || session.user.telegramAuth?.photoUrl}
                />
                <span className="font-medium">{session.user?.username || session.user?.displayName || session.user?.email}</span>
              </div>
              <Button
                className="w-full"
                onClick={() => router.push('/')}
