...
import { useCreateWithdrawal, useTransactionTracker } from '@/hooks/transactions';
import { withdrawFromPool } from '@/lib/stacks';
import { formatSTX, formatPLMD, calculateWithdrawalAmount } from '@/lib/formatters';
  ...
  const handleWithdrawal = async () => {
    ...
    try {
      ...
      if (!withdrawalResult.success || !withdrawalResult.data?.transaction) {
        throw new Error(withdrawalResult.message || 'Failed to create withdrawal record');
      }
          ...
          {isLoadingPool ? (
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                ...
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground">Your Pool Share Value</span>
                  <span className="font-medium">{formatSTX(userBalance?.poolShareValue || '0')} STX</span>
                </div>
              </CardContent>
            </Card>
          ) : null}
          ...
          {withdrawalPreview && (
            <Card>
              ...
              <CardContent className="space-y-3">
                ...
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground">Exit Fee ({poolInfo?.exitFeeRate}%)</span>
                  <span className="text-red-600">-{withdrawalPreview.feeFormatted} STX</span>
                </div>
                <Separator />
                <div className="flex justify-between">
                  <span className="text-muted-foreground">STX You'll Receive</span>
                  <span className="font-bold text-green-600">
                    {withdrawalPreview.netAmountFormatted} STX
                  </span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground">PLMD Tokens Burned</span>
            ...
            <Card className="border-red-200 bg-red-50">
              <CardContent className="p-2">
                <div className="text-xs text-red-800">
                  ...
                  <p>You only have {formatPLMD(userBalance?.plmdBalance || '0')} PLMD tokens available.</p>
                </div>
              </CardContent>
            </Card>
          )}
          ...
          <Card className="border-yellow-200 bg-yellow-50 p-2">
            <CardContent className="p-2">
              <div className="text-xs text-yellow-800">
                ...
                <p>
                  Withdrawals are irreversible. The STX amount you receive depends on the current NAV
                  and may be less than your original deposit if the pool has losses.
                </p>
              </div>
            </CardContent>
          </Card>
