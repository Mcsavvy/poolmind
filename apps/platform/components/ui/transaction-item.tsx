...
import { formatSTX, formatPLMD, formatDate, formatTxHash, formatRelativeTime } from '@/lib/formatters';
...
const getTransactionBlockHeight = (transaction: Transaction): number | undefined => {
...
const getTokensBurned = (transaction: Transaction): string | undefined => {
  return transaction.withdrawalMetadata?.tokensBurned || transaction.withdrawalMetadata?.poolSharesBurned;
};
...
export function TransactionItem({ 
  ...
  return (
    <div className={`flex items-center justify-between p-4 border rounded-lg hover:bg-muted/50 transition-colors ${className}`}>
