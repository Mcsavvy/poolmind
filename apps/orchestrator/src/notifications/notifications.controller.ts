...
export class NotificationsController {
  ...
  @ApiQuery({
    name: 'priority',
    required: false,
    enum: NotificationPrioritySchema.options,
  ...
  async getInAppNotifications(
    ...
    try {
      ...
      this.logger.debug(
        `Getting in-app notifications for user ${user._id} with query:`,
        validatedQuery,
      );

      const result =
        await this.notificationsService.getInAppNotificationsForUser(
          user._id.toString(),
          {
          ...
          metadata: notification.metadata
            ? {
                ...
                data: notification.metadata.data
                  ? Object.fromEntries(notification.metadata.data)
                  : undefined,
              }
            : undefined,
      ...
      this.logger.error(
        `Failed to get in-app notifications for user ${user._id}:`,
        error,
      );
  ...
  async getUnreadCount(
    ...
    try {
      ...
      this.logger.error(
        `Failed to get unread count for user ${user._id}:`,
        error,
      );
      throw new HttpException(
  ...
  async markAsRead(
    ...
    try {
      const success =
        await this.notificationsService.markInAppNotificationAsRead(
          notificationId,
          user._id.toString(),
        );
      ...
      this.logger.error(
        `Failed to mark notification ${notificationId} as read for user ${user._id}:`,
        error,
      );
      throw new HttpException(
  ...
  async markAllAsRead(
    ...
    try {
      ...
      const markedCount =
        await this.notificationsService.markAllInAppNotificationsAsRead(
          user._id.toString(),
  ...
  async deleteNotification(
    ...
    try {
      const success =
        await this.notificationsService.deleteInAppNotificationForUser(
          notificationId,
          user._id.toString(),
        );
      ...
      this.logger.error(
        `Failed to delete notification ${notificationId} for user ${user._id}:`,
        error,
      );
      throw new HttpException(
  ...
  async bulkAction(
    ...
    try {
      ...
      switch (validatedBody.action) {
        ...
        default:
          throw new BadRequestException(
            `Unsupported bulk action: ${validatedBody.action}`,
          );
      }
      ...
      this.logger.error(
        `Failed to perform bulk action for user ${user._id}:`,
        error,
      );
      throw new HttpException(
  ...
  @ApiOperation({
    summary: 'Create in-app notification (Admin/Moderator only)',
  })
  @ApiResponse({
    status: 201,
  ...
  async createInAppNotification(
    ...
    try {
      ...
      this.logger.error(
        `Failed to create in-app notification by user ${user._id}:`,
        error,
      );
      throw new HttpException(
  ...
  async cleanupOldNotifications(@Query('daysOld') daysOld?: number): Promise<{
    success: boolean;
    ...
    try {
      ...
      const result =
        await this.notificationsService.cleanupOldInAppNotifications(days);
  ...
  async queueInAppNotification(
    ...
    try {
      ...
      this.logger.error(
        `Failed to queue in-app notification by user ${user._id}:`,
        error,
      );
      throw new HttpException(
