    ...
    try {
      ...
      switch (data.type) {
        ...
        case 'broadcast':
          ...
          result = await this.notificationsService.sendToAllUsers(data.message);
          break;

        case 'channel':
          targetInfo = 'channel';
          result = await this.notificationsService.sendToChannel(data.message);
          break;
        ...
        default:
          throw new Error(
            `Unknown notification job type: ${(data as any).type}`,
          );
      }
