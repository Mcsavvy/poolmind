...
export class BalanceController {
  ...
  /**
   * @swagger
   * /api/v1/balance/address/{address}/plmd:
   *   get:
   *     summary: Get PLMD token balance for a specific address
   *     description: Retrieve the PoolMind token balance for any STX address (accessible by bots and users)
   *     tags: [Balance]
   *     security:
   *       - bearerAuth: []
   *       - hmacAuth: []
   ...
   *         schema:
   *           type: string
   *         description: STX address to query
   *         example: SP2J6ZY48GV1EZ5V2V5RB9MP66SW86PYKKQ9H6DPR
   *     responses:
   *       200:
   *         description: PLMD balance retrieved successfully
   ...
   *         description: Server error
   */
  static async getPlmdBalanceByAddress(req: Request, res: Response): Promise<void> {
    try {
      ...
      console.error("Error fetching PLMD balance by address:", error);
      res.status(500).json({
        success: false,
        message:
          error instanceof Error
  ...
  /**
   * @swagger
   * /api/v1/balance/address/{address}/stx:
   *   get:
   *     summary: Get STX balance for a specific address
   *     description: Retrieve the STX balance for any STX address (accessible by bots and users)
   *     tags: [Balance]
   *     security:
   *       - bearerAuth: []
   *       - hmacAuth: []
   ...
   *         description: Server error
   */
  static async getStxBalanceByAddress(req: Request, res: Response): Promise<void> {
    try {
      const { address } = req.params;
      ...
      console.error("Error fetching STX balance by address:", error);
      res.status(500).json({
        success: false,
        message:
          error instanceof Error
  ...
  /**
   * @swagger
   * /api/v1/balance/address/{address}/all:
   *   get:
   *     summary: Get complete balance information for a specific address
   *     description: Retrieve both PLMD token balance and STX balance for any STX address (accessible by bots and users)
   *     tags: [Balance]
   *     security:
   *       - bearerAuth: []
   *       - hmacAuth: []
   ...
   *         description: Server error
   */
  static async getAllBalancesByAddress(req: Request, res: Response): Promise<void> {
    try {
      const { address } = req.params;
      ...
      console.error("Error fetching balances by address:", error);
      res.status(500).json({
        success: false,
        message:
          error instanceof Error ? error.message : "Failed to fetch balances",
